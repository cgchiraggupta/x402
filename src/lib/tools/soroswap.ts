...
import { horizonServer, NETWORK_PASSPHRASE, getUSDCIssuer } from "../stellar/client";
...
export async function build_swap_tx(
  ...
  try {
    ...
    const minReceive = (parseFloat(quote.toAmount) * (1 - slippagePercent)).toFixed(7);
 ...
 */
export async function get_token_price(token: string, vsToken: string = "USDC"): Promise<number> {
  try {
...
export function validate_swap_params(
  ...
  if (!validTokens.includes(from_token)) {
    return { valid: false, error: `Invalid from token: ${from_token}. Supported: ${validTokens.join(", ")}` };
  }
  
  if (!validTokens.includes(to_token)) {
    return { valid: false, error: `Invalid to token: ${to_token}. Supported: ${validTokens.join(", ")}` };
  }
